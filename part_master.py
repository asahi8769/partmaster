...
if __name__ == "__main__":
    ...
    for d_type in ['exp', 'dom']:
        _, df_aux = Master.run(d_type=d_type)
        grouped_df_1, grouped_df_3, master_trimmed = RankFilter(d_type).grouping()

        with pd.ExcelWriter(rf'files\spawn\{d_type}_부품선정.xlsx') as writer:
            master_trimmed.to_excel(writer, sheet_name=f'master+{d_type}', index=False)
